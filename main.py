...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(
            "ULAgent LogManager sim"
        )

        # self.resize(800, 600)
        # 最小サイズに設定 自動で適切な大きさになる
        self.resize(100, 100)
    ...
    def load_scenario(self, scenario_name=None):
        ...
        try:
            ...
            if not all(
                    ...
                    for value in sampling_data.values()):

                raise TypeError(
                    "sampling values must be numbers"
                )
            ...
            severity_index = self.severity_combo.findText(
                severity
            )

            if connection_index < 0:
                raise ValueError(
                    f"unsupported connection: {connection}"
                )

            if chamber_index < 0:
                raise ValueError(
                    f"unsupported chamber: {chamber}"
                )
        ...
        self.current_sampling = sampling_data
